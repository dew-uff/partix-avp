...
public class SubQuery {
	...
	protected boolean sameQuery; // indica se as sub-consultas pertencem a mesma query original. Usada para as sub-consultas geradas a partir de colecoes.
	protected String constructorElement = ""; // define o elemento especificado pelo usuario para a estrutura da resposta. Ex.: <results><el1></el1>...</results>. Neste caso, o elemento  <results>.
	protected String elementAfterConstructor = ""; // define o elemento apos o construtor para a estrutura da resposta. Ex.: <results><order><el1></el1>...</order></results>. Neste caso, o elemento  <order>.
	...
	private boolean updateOrderClause = true; // usado para identificar se ha elementos em torno do elemento do order by e acertar a clausula que ser utilizada no retorno.

    private boolean isOnlyConnection; // usado para identificar se ao executar a subconsulta o resultado parcial sera gravado direto na colecao ou se antes passara pelo sistema de arquivos
	...
	/* Metodo para execucao das sub-consultas geradas na fragmentacao virtual simples */
	//Luiz Matos - adicionado atributo 'onlyCollection' que se for true nao fara a gravacao do resultado parcial no sistema de arquivos
	public static boolean executeSubQuery(String xquery, Query q, SubQuery sbq, Database db, boolean onlyCollectionStrategy, String nodeName, QueryExecutor qe) throws SubQueryExecutionException {
		//LOG.debug("executeSubQuery() xquery: " + xquery);
		//System.out.println("executeSubQuery() xquery: " + xquery);
		boolean hasResults = false;
		boolean addheader = true;
		XQResultSequence rs = null;
		...
		try {
			internalQuery = getInternalQuery(xquery);

			// execute the internal query
			//medir tempo execucao subquery
			long startExecTime = System.nanoTime();
			rs = db.executeQuery(internalQuery);
			long elapsedExecTime = System.nanoTime() - startExecTime;
			
			qe.setTimeExecSubquery(elapsedExecTime / 1000000); // em ms
			
			//Nao passara pelo sistema de arquivos
			//if (onlyCollectionStrategy) {
				//Inicializa a string com o resultado parcial 
				String partialResult = "";
				//deleteCollection(db);
				
				//medir a montagem do resultado parcial na string partialResult
				long startBuildingPartialResultTime = System.nanoTime();
				while (rs.next()) {
					...
					if (addheader) {
						...
						// write XML header
						partialResult = getTitle();
						
						// write partial result root element
						partialResult += PARTIAL_BEGIN_ELEMENT;
				...
				if (hasResults) {
					// close partial result constructor element
					String footer = sbq.getConstructorElement().replace("<", "</");
					partialResult += footer;
					partialResult += "\r\n";

					if (!q.isOrderByClause()) { // se a consulta original nao possui order by adicione o elemento idOrdem
						String partialOrderElement = PARTIAL_IDORDEM_BEGIN_ELEMENT + getIntervalBeginning(xquery,sbq) + PARTIAL_IDORDEM_END_ELEMENT;
						partialResult += partialOrderElement;
					}

					// write partial result root ending element
					partialResult += PARTIAL_END_ELEMENT;
					long elapsedBuildingPartialResultTime = System.nanoTime() - startBuildingPartialResultTime;
					qe.setTimeBuildingPartialResult(elapsedBuildingPartialResultTime / 1000000); // em ms
					
					// forces output of all data before returning
					//Insere partialResult em uma colecao temporaria, sem passar pelo sistema de arquivos
					//medir gravacao do resultado parcial na colecao temporaria

					long startInsertingTempCollectionTime = System.nanoTime();
					//db.loadXMLStringInCollection(AVPConst.TEMP_COLLECTION_NAME, "partial_" + System.currentTimeMillis(), partialResult, nodeName);
					db.loadXMLStringInCollection(qe.getTempCollectionName(), "partial_" + System.currentTimeMillis(), partialResult, nodeName);
					long elapsedInsertingTempCollectionTime = System.nanoTime() - startInsertingTempCollectionTime;
					qe.setTimeInsertingTempCollection(elapsedInsertingTempCollectionTime / 1000000); // em ms
				
				}
				db.freeResources(rs);
				partialResult = "";
//			} else { //no caso de onlyCollectionStrategy == false, fazendo com que os resultados parciais sejam gravados em disco
//				hasResults = saveOutputFileSystem(xquery, q, sbq, db, rs, out);
//			}
		} catch (IOException e) {
	...
	//Luiz Matos
	private static boolean saveOutputFileSystem(String xquery, Query q, SubQuery sbq, Database db, XQResultSequence rs, OutputStream out) throws SubQueryExecutionException, XQException, IOException {
	...
	public static void deleteCollection(Database db) throws IOException {
		...
		try {
			...
			// collection
			db.createCollection(AVPConst.TEMP_COLLECTION_NAME);
	...
	public static String getElementAfterConstructor(String query) {
		int returnPos = query.toUpperCase().indexOf("RETURN ");
				...
				if ( posSlash != -1 ) {
					...
					if (subOrder.trim().indexOf(elementAfterConstructor) !=-1 && subOrder.trim().indexOf(orderElements[i])!=-1) {
						...
						if ( subOrder.trim().indexOf("<")  != -1 ) { // se houver elementos entre o elemento depois do construtor e o elemento do order by, obtenha-o.
							elementAroundOrderBy = subOrder.trim().substring(subOrder.trim().indexOf("<")+1, subOrder.trim().indexOf("{")); // obtem os elementos antes do elemento do order by.Ex.:<date><sp>{$order/ship_date}</sp></date>. Retornaria date><sp>.							
	...
	public static String getConstructorElement(String xquery){
		//System.out.println("getConstructorElement: " + xquery);
		...
		//System.out.println("constructorElement = " + constructorElement);	
		return constructorElement;
	}
	
	public boolean isOnlyConnection() {
		return isOnlyConnection;
	}

	public void setOnlyConnection(boolean isOnlyConnection) {
		this.isOnlyConnection = isOnlyConnection;
	}
