package mediadorxml.algebra.basic;
...
public class TreeNode implements Cloneable, Serializable {
	...
	protected int nodeId;   		 		// Identificador único do nodo (logical class id)
	protected String label; 	 		// Conteúdo do nodo
	...
	protected Predicate predicate;		// Predicado (seleção)
	protected boolean isKeyNodeFlag;		// Flag que indica se o nodo é tipo chave (associado a uma variável) da APT
	...
	/*
	 * Atenção: o NodeId deve ser gerado pelo IdGenerator
	 */
	...
	public TreeNode getKeyNode(){
		...
		if (rootNode.isKeyNode()){
			...
			while (rootNode.hasChield()){
				rootNode = rootNode.getChild(0); // Um key node nunca terá irmãos
				if (rootNode.isKeyNode()){
					return rootNode;
				}
			}
	...
	/*
	 *  Comparação de treeNodes para verificar se uma satisfaz as condições da outra
	 */	
	public boolean satisfies(final TreeNode onode){
		boolean satisfies = true;
		
		if (this.predicate != null){
			// Comparação do predicado do nó
		...
		if ((satisfies) && (this.hasChield())){
			for (int i=0;i<this.getChildren().size();i++){
				...
				if (!satisfies){
					return false;  // Se algum dos nodos filhos não satisfizer, podemos retornar false
				}
			}
		}	
	...
	public String toString(){
		...
		// inclusão do nível na hierarquia
...
//		// Implementação do deep clone por serialização em xml
