...
import mediadorxml.algebra.basic.TreeNode.MatchSpecEnum;
import mediadorxml.algebra.basic.TreeNode.RelationTypeEnum;
...
public abstract class AbstractOperator implements Cloneable, Serializable {
	...
	protected int operatorId;   	// Identificador único do operador no plano
	...
	protected WrapperLocation executionSite; // Site onde o operador será executado
	protected int estimNodesProces; // Estimativa de nodos processados pelo operador (para estimativa de custo)
	protected boolean localizationOperator; // Indicador se o operador foi gerado na etapa de localização da query para composição dos fragmentos
	
	protected ArrayList<AbstractOperator> childOperators;
	protected AbstractOperator parentOperator;
	
	static private final Logger logger = Logger.getLogger(AbstractOperator.class);
	...
	public void addConstructOperator() throws IOException, OptimizerException{
		...
		// Alteração do root node caso o operador seja um Union ou Join
...
//		 Construção da APT do Select com root = key node LCL
		...
		// Remoção dos nodos duplicados no Construct/Select
		...
		// Inclusão do Construct e do Select na árvore de operadores
		this.insertBelowParent(construct);
		this.insertBelowParent(select);
		
		// Alteração da especificação de cardinalidade do operador de Select para 
		// forçar a sub-query ser um FOR e não um LET
	...
	private void searchAndIncludeUsedNodesInAptSelect(AbstractOperator operator, TreeNode k, PatternTree aptS) throws IOException, OptimizerException {
		...
		// Avaliação do nodo do Construct para verificação se o return não é do tipo $x
	...
	private void removeDuplicatedNodes(TreeNode nodeConstruct, TreeNode nodeSelect){
		...
		if (nodeConstruct.hasChield()){
			// Buscamos irmãos duplicados e removemos um deles dos filhos do nó principal
			for (int c=0; c<nodeConstruct.getChildren().size(); c++){
				...
				//Buscaremos nos outros irmãos algum dublicado
				for (int i=c+1; i<nodeConstruct.getChildren().size(); i++){
					...
					if (label.equals(labelIrmao)){
						// Se for igual, removemos o irmão
						...
						// Remoção do nodo equivalente do Select
	...
	protected static void changeNodeLabelWithLCL(TreeNode node){
		...
		// Nodos com filhos não podem ser LCL
		if (!node.hasChield()){
	...
	public AbstractOperator clone() throws CloneNotSupportedException{
		// Implementação do deep clone por serialização em memória
