...
public class SimpleVirtualPartitioning {
	...
	protected int numberOfNodes; // n�mero de n�s dispon�veis para o processamento da consulta
	...
	protected int cardinalityOfElement; // cardinalidade do elemento utilizado para a fragmenta��o
	 ...
	 * mas que cont�m o subcaminho utilizado para a fragmenta��o, a fim de evitar intervalos conflitantes e, consequentemente, 
	...
	public void setAddedPredicates(String completePath, Integer elementCardinality) {
		...
		this.addedPredicates.put(completePath,elementCardinality); //caminho completo que define a vari�vel XML e cardinalidade do elemento usada para criar os fragmentos.
	}
	...
	public String getSelectionPredicate(int index, String fragmentationVariable) {
		...
		if ( index == - 1 && this.selectionPredicates.get(fragmentationVariable) == null ) { // Se �ndice igual a -1, sistema ir� gerar os intervalos pela primeira vez.				
			...
			if ( fragmentSize > 0 ) { // cardinalidade do elemento � maior que a quantidade de n�s de processamento.
			...
			else { // n�mero de elementos � menor que o n�mero de processadores. 
		...
		else { // Sistema j� conhece todos os intervalos, devolve apenas os predicados � medida que as sub-consultas s�o geradas
	...
	public void addVirtualPredicates(String subPath, String variableName, Integer elementCardinality) throws IOException {
		...
		if (svp.isNewDocQuery()){
			fragments = null; // reinicializa o arrayList para n�o utilizar os predicados j� adicionados da sub-consulta anterior.
		}
		...
		for ( int j = 0; j < pred.size(); j++ ) {
			...
			if ( fragments == null ) {								
				...
				for ( String frag : fragments ) {
					...
					// Verifica se o caminho at� o atributo de fragmenta��o j� foi usado para gera��o de algum predicado de sele��o.
					// Em caso afirmativo retorna a cardinalidade do atributo de fragmenta��o no documento, caso contr�rio retorna zero.
					Integer cardinality = pathAlreadyExists(virtualFragment);								
					
					// se nenhum predicado de sele��o foi adicionado para o mesmo caminho xpath, adicione todos os intervalos
					if ( cardinality == 0 ) { 									
					...
					else { // adicionar apenas os intervalos cujas sub-consultas geradas n�o produzem resultados vazios.
						/*ex.: for $x in doc('loja','informacoesLoja')/Loja/Itens/Item[position >= 1 and position < 18]
							   ...
							   O caminho /Loja/Itens/Item j� teve um predicado de sele��o adicionado, logo o intervalo da vari�vel
							   $a subsequente, deve adicionar os mesmo intervalos do predicado anterior e n�o predicados disjuntos,
					    ...
					    */
						...
						if ( cardinality == elementCardinality ) { // mesmo caminho e mesmo elemento para FORs diferentes na consulta																													
							...
							if (virtualFragment.substring(0, 1).equals(Integer.toString(j))) {						
								virtualFragment = virtualFragment.replace(completePath, completePath + predicate);
								virtualFragment = (j) + virtualFragment;
								// Adicionar o predicado na posi��o correta. Como explicado no coment�rio acima.
								fragmentsBkp.add(virtualFragment);
							}
						}
						else {
		...
		if ( fragmentsBkp!=null && fragmentsBkp.size()>0 ) { // � diferente de null quando h� mais de um FOR na consulta informada pelo usu�rio.
	...
	/**
	 * Fun��o que retorna o caminho que caracteriza o elemento escolhido como atributo de fragmenta��o
	 * @param completePath String do FOR at� o t�rmino do caminho Xpath do FOR que est� sendo analisado.
	 * @return O caminho at� o atributo de fragmenta��o
	 */
	public String getPathPredicate(String completePath) {
				
		int posParenthese = completePath.indexOf(")"); // Fim da defini��o do documento/cole��o		
	...
	/**
	 * Fun��o que retorna a cardinalidade do elemento que j� teve o predicado adicionado, ou caso n�o encontre, retorna zero.
	 * @param forClausePath String do vari�vel do FOR at� o t�rmino do caminho Xpath do FOR que est� sendo analisado.
	 * @return A cardinalidade do atributo de fragmenta��o ou zero, caso este atributo ainda n�o tenha sido usado em predicados anteriores.
	 */
	public Integer pathAlreadyExists(String forClausePath){
		
		Hashtable<String, Integer> addePredicates = this.getAddedPredicates(); // Obt�m os predicados j� adicionados a consulta original.
		...
		while ( iter.hasNext() && cardinality == 0 ){ // enquanto houver predicados adicionados e nenhum deles for semelhante ao predicado atual, continue.	
			...
			String pathCompareTo = getPathPredicate(completePath); // GetPathPredicate retorna apenas a parte xpath de completePath passado como par�metro.
		...
		if (fragmentsList != null) {
			...
			for ( String frag: fragmentsList ){ 
				...
				if ( frag.toUpperCase().indexOf("FOR ") != -1 ) {
					// Obt�m a string antes do primeiro FOR
					strBeforeFor = frag.substring(0, frag.toUpperCase().indexOf("FOR "));
				}	
				else if ( frag.toUpperCase().indexOf("LET ") != -1 ) {								
					// Obt�m a string antes do primeiro LET
					strBeforeFor = frag.substring(0, frag.toUpperCase().indexOf("FOR "));							
				}
				
				if ( strBeforeFor.length() > 0 ) {
					// Retira todos os d�gitos que indicam o �ndice do predicado, adicionado na consulta para excluir os fragmentos que geram resultado vazio.
	...
	public String getSelectionPredicateToCollection(String fragmentationVariable, String virtualAttribute, String mainQuery) {
		...
		if (!mainQuery.toUpperCase().contains("FOR $")) {
			if (fragmentationVariable.charAt(fragmentationVariable.length()-1) != ' ') { // se �ltimo caracter n�o for espa�o, o usu�rio usou uma cl�usula LET, cuja vari�vel est� imediatamente antes dos caracteres := 
		...
		SubQuery sbq = SubQuery.getUniqueInstance(true);
		// Obt�m o caminho at� onde o elemento de fragmenta��o.
