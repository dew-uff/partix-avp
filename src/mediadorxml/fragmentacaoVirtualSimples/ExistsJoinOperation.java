...
public class ExistsJoinOperation {
	...
	public void verifyInputQuery() {
		...
		int pos = -1;
		String tmp = "";
		int numberForClauses = 0; // n�mero de cla�sulas FOR		
		
		pos = this.getXquery().toUpperCase().indexOf("FOR $"); // posi��o da primeira cl�usula FOR.
		...
		while ( pos >= 0 ) {		
			...
			//pos = this.getXquery().toUpperCase().indexOf("FOR $"); // posi��o da primeira cl�usula DOC().						
			tmp = this.getXquery().substring(pos+5, this.getXquery().length()); // Obt�m a string ap�s a defini��o de for.
						
			this.setXquery(tmp);
			pos = this.getXquery().toUpperCase().indexOf("FOR $"); // posi��o da pr�xima cl�usula FOR, se houver.*/			
	...
	/* Recebe como par�metros os caminhos dos join */
	public void verifyJoins(String pathJoin1, String pathJoin2, String varJoin1, String varJoin2, String atrJoin1, String atrJoin2) throws IOException{		
		...
		if (forClauses.containsKey("$"+varJoin1)){ // verifica se a vari�vel das jun��es j� existem na hashtable, de forma a substituir o nome da vari�vel pelo caminho que ela representa.			
			pathJoin1 = pathJoin1.replace("$"+varJoin1, q.getPathVariable("$"+varJoin1));
		}
				
		if (forClauses.containsKey("$"+varJoin2)){ // verifica se a vari�vel das jun��es j� existem na hashtable, de forma a substituir o nome da vari�vel pelo caminho que ela representa.			
		...
		if ( cardinality !=null && cardinality2!=null && Integer.parseInt(cardinality) > 0 && Integer.parseInt(cardinality2) > 0) { // especificou o caminho completo
			// Verificar se o atributo que possui a menor cardinalidade, uma vez que as chaves prim�rias, em geral, possuem menor 
			...
			if ( cardinality != null && Integer.parseInt(cardinality) > 1) { // s� pode fragmentar se houver rela��o 1:N.				
			...
			if ( cardinality2 != null && Integer.parseInt(cardinality2) > 1) { // s� pode fragmentar se houver rela��o 1:N.
