...
public class SimplePathExpr extends Clause {
		...
		if (element == "Slash"){
			...
			try {
				...
				if (!q.isElementConstructor() && !q.isAddedPredicate() && 
						...
						(q.getqueryExprType()!=null && !q.getqueryExprType().contains("collection")))) { // Se nao for o caminho xpath que representa a estrutura do resultado especificada pelo usu�rio, verifique a cardinalidade.
					...
					if ( !q.isExistsJoin() 
						...
						if ( q.getLastReadCardinality()==-1 || q.getLastReadCardinality()==0 || q.getLastReadCardinality()==1 ){
							...
							if ( q.getLastReadCardinality()==-1 || q.getLastReadCardinality()==1 ) { // especificou o caminho completo
								...
								int cardinality = 0;
                                try {
                                    cardinality = cm.getDatabase().getCardinality(this.xpath, q.getLastReadDocumentExpr(), (q.getLastReadCollectionExpr()!=null && !q.getLastReadCollectionExpr().equals("")?q.getLastReadCollectionExpr():""));
                                } catch (XQException e) {
                                    // TODO Auto-generated catch block
                                    e.printStackTrace();
                                }
							    
								// Condi��o para selecionar o atributo de fragmenta��o: Obter o primeiro elemento no caminho xpath da consulta, cuja cardinalidade no documento XML � maior que 1.
								if ( cardinality >1 ) {
									
									if ( !q.isAddedPredicate() ) { // Se ainda n�o adicionou nenhum predicado no caminho xpath da vari�vel em quest�o.
										...
										q.setAddedPredicate(true); // Evita a adi��o do mesmo predicado nos sub-elementos restantes
										
										int posForClause = q.getInputQuery().indexOf(q.getFragmentationVariable()); // posicao da variavel do FOR atual
										int posPath = q.getInputQuery().indexOf(this.xpath); // posicao onde come�a a defini�ao do caminho
										...
										if (posPath < posForClause){
											...
											posPath = posForClause + substring.indexOf(this.xpath); // somo a posicao onde come�ao o caminho onde estou com os caracteres ate chegar no FOR que estou analisando para corrigir a contagem, considerando a consulta original.
										}
								...
								if ( sbq.getSubQueries()!=null && sbq.getSubQueries().size() > 0 ){
									// j� fragmentou a consulta. N�o fa�a nada.
	...
	public void analyzeAncestral(String collectionName, String docName, String varName, String element) throws IOException{
		...
		if (docName!=null && !docName.equals("")) { // se especificou o nome de um documento dentro de uma cole��o. Ex.: doc('x.xml','y')
		...
		if (docName!=null && !docName.equals("")) { 
			...
			try {
                cardinality = cm.getDatabase().getCardinality(parentNode, docName, (collectionName!=null && !collectionName.equals("")?collectionName:""));
            } catch (XQException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
		    
			if (cardinality == 1 ) { // elemento pai tem cardinalidade 1, pode executar a fragmenta��o. Setar a cardinalidade para 0 de forma a entrar no comando while seguinte.
				cardinality = 0; 
			}
		}		
		...
		if ( parentNode!= null && !parentNode.equals("") && !parentNode.contains("Erro") && !q.isAddedPredicate()) {
			...
			while ( parentNode!= null && !parentNode.equals("") && !parentNode.contains("Erro") && cardinality == 0){				
				...
				try {
                    cardinality = cm.getDatabase().getCardinality(q.getAncestralPath(), docName, (collectionName!=null && !collectionName.equals("")?collectionName:""));
                } catch (XQException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                }
			...
			if ( cardinality > 1 ) {
				...
				while (cardinality > 1 && !completePath.equals("") && !completePath.equals("")) {				
					...
					if ( posSlash >= 0) {
						...
						try {
                            cardinality = cm.getDatabase().getCardinality(completePath, 
                                    docName, 
                                    (collectionName!=null && !collectionName.equals("")?collectionName:""));
                        } catch (XQException e) {
                            // TODO Auto-generated catch block
						...
						//cardinality = ExecucaoConsulta.executeQuery(xquery);
						try {
                            cardinality = cm.getDatabase().getCardinality((!completePath.equals("")?completePath+"/":completePath) + completePathTmp, 
                                    docName, 
                                    (collectionName!=null && !collectionName.equals("")?collectionName :""));
                        } catch (XQException e) {
                            // TODO Auto-generated catch block
				...
				//cardinality = ExecucaoConsulta.executeQuery(xquery);	
				try {
                    cardinality = cm.getDatabase().getCardinality(
                            (!completePath.equals("")?completePath+"/":completePath) + completePathTmp, 
                            docName, 
                            (collectionName!=null && !collectionName.equals("")?collectionName:""));
                } catch (XQException e) {
                    // TODO Auto-generated catch block
				...
				q.setAncestralPath(""); // Quando a fragmenta��o j� tiver sido efetuada, n�o ser� necess�ria analisar os elementos subsequentes.
				q.setAddedPredicate(true);
				q.setLastReadCardinality(cardinality);
			}
			else {				
