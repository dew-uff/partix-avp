		...
		if (element.equals("VarName")){
			...
			if (this.isWaitingVarNameFunction()) {
                ...
                if (q.getLastReadForLetVariable().equals("")) { // funcao de agregacao apenas na clausula let, nao ha clausula For na consulta						
                	q.setAggregateFunctions(q.getLastReadFunction(), "$"+node.getText(), ""); // ultimo parametro � vazio, pois a consulta deseja obter somente o resultado da fun��o, n�o h� restri�oes tal como count($c)>5.
                }
                else {
                	String value = q.getAggregateReturn().get("$"+node.getText());
                	value = value + ":" + q.getLastReturnVariable();					
                	q.setAggregateFunctions(q.getLastReadFunction(), "$"+node.getText(), value); // ultimo parametro � vazio, pois a consulta deseja obter somente o resultado da fun��o, n�o h� restri�oes tal como count($c)>5.
                }
