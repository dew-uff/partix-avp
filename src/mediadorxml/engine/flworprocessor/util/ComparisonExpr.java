...
public class ComparisonExpr extends Clause {
	...
	protected String _function; 	 // Operador da função do lado esquerdo da comparação
	protected TreeNode _node;		 // path do lado esquerdo da comparação
	protected TreeNode _nodeJoin;	 // path do lado direito da comparaçao (quando for join)
	protected String _joinPredicate; // predicado da junção em relação aos LCLs
	...
	protected void processSimpleNode(SimpleNode node, boolean debug){
		...
		if (element.equals("VarName")){		
			...
			// Verificação se é a primeira variável (se forem duas é join)
			if (this._node == null){
				// Criação de node com o nome da variável				
				this._node = new TreeNode(node.getText(), TreeNode.RelationTypeEnum.ROOT);			
			}
			// Segunda variável = join
			else{
				// Predicado de junção
				// Criação de node com o nome da variável
