...
public class ForLetClause extends Clause {
	...
	public void compileForLet(SimpleNode node, boolean debug){	
        ...
        if (!forClauses.containsKey(q.getFragmentationVariable())){ // verifica se a vari�vel j� existe na hashTable
	...
	protected void processSimpleNode(SimpleNode node, boolean debug){
		...
		if ("ForClause".equals(element)){					
                ...
                q.setFragmentationVariable("$"+newNode.getLabel()); // indica a �ltima vari�vel XML lida, referente a um FOR.					
			
		}
		if ("LetClause".equals(element)){
			final TreeNode newNode = new TreeNode(((SimpleNode)node.jjtGetChild(0)).getText(), TreeNode.RelationTypeEnum.ROOT);
			Query q = Query.getUniqueInstance(true);
            q.setLastReadLetVariable("$"+newNode.getLabel()); // Vari�vel entre o let e o := 				
		}		
		...
		if ("VarName".equals(element)){
            ...
            if (!q.getLastReadDocumentExpr().equals("")) // se nome do documento j� foi preenchido, o pr�ximo texto � a defini��o do nome da cole��o.
            	q.setLastReadCollectionExpr(node.getText());
            				
            // newNode.getLabel() sempre consome a extens�o .xml, no entanto, se na consulta original estiver especificado a extens�o
            // esta ser� necess�ria para consultar a cardinalidade dos elementos do caminho xpath.
		...
		else if ("PathExpr".equals(element)){  // Referente a uma outra vari�vel j� declarada
			...
			// Marca��o do nodo correspondente � vari�vel
