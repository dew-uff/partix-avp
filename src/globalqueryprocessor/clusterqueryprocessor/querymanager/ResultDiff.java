...
public class ResultDiff {
	...
	private XQException exception;
	private int updateCount = -1;

	public void add(int nodeId, int updateCount, XQException exception) {
		if(exception == null) { 
	...
	public void compare() {
		if(resultHistogram.size() == 1 && exceptionResultList.size() == 0) {
			...
			exception = new XQException(error); 
	...
	public XQException getException() {
		return exception;
	}
	...
	class UpdateResult {
		...
		public UpdateResult(int nodeId, XQException exception) {
			this.nodeId = nodeId;
