...
public class ClusterQueryProcessorAdmin {
	...
	public static void main(String[] args) {
		...
		if (args.length < 3) {
			System.out.println("usage: java ClusterQueryProcessorAdmin ServerName PortNumber op ");
		...
		try {
			if (args[2].toUpperCase().equals("SHUTDOWN")) {
				...
				setClusterSize(args[0].trim(), args[1].trim(), Integer.parseInt(args[3].trim()));
			} else {
	...
	public static void shutdown(String serverName, int portNumber)
		...
		qp = (ClusterQueryProcessor) MyRMIRegistry.lookup(serverName,portNumber,"//" + serverName + ":" + portNumber + "/ClusterQueryProcessor");
		qp.shutdown();
	...
	public static void setClusterSize(String serverName, String portNumber,
		...
		cqp = (ClusterQueryProcessor) Naming.lookup("//" + serverName + ":" + portNumber + "/ClusterQueryProcessor");
