package localqueryprocessor.dynamicrangegenerator.avp;
...
public class AdaptiveVirtualPartitioning {
	...
	//Variáveis by Luiz Matos
	private final int ini_part_sz = 1024; //First partition size used in algorithm execution
	private final float ini_sz_growth_tx = (float) 1.0; //First tax used for partition size increase
	private final float sz_decrease_tx = (float) 0.75; //Tax used for partition size reduction when performance deterioration isdetected
	private final float restarting_sz_growth_tx = ini_sz_growth_tx*(float)0.2; //Partition size increase tax used when restarts increasing partition size after performance deterioration detection
	...
	public AdaptiveVirtualPartitioning(String initialSubQuery) throws IOException{
		...
		System.out.println("initialFragment: " + initialFragment);
	...
	private void adjustFragmentSize() throws IOException{
		...
		while ( this.getLastEnding() < this.getEndOfInterval()) { // enquanto o final do ultimo fragmento processado for menor que o final do fragmento original gerado na FVS, prossiga com a fase de ajuste.
			...
			if ( this.getCurrentSize() == -1 ){ // ainda nao processou nehum fragmento
				this.setCurrentSize(ini_part_sz); //First partition size (Luiz Matos)
			}
