...
//    private final int INITIAL_NUMBER_OF_ELEMENTS = 2; // number of keys to the first size that will be tested
//
//    private final float INITIAL_SIZE_GROWTH_TAX = (float) 4.0; // initial tax to increase the interval
//    private final float RESTART_SIZE_GROWTH_TAX = INITIAL_SIZE_GROWTH_TAX * (float) 1.0; // increase fragment tax after deterioration detected
//    private final float REDUCTION_IN_SIZE_GROWTH_TAX = (float) 0.25; // reduction fragment tax when deterioration is detected 
//    private final float MIN_SIZE_GROWTH_TAX = (float) 0.5;
//    private final float SIZE_REDUCTION_WHILE_RESTARTING = (float) 0.5;

    private final int NUMBER_OF_EXECUTIONS = 1; // number of executions to test each different size
    private final int INITIAL_NUMBER_OF_ELEMENTS = 1024; // number of keys to the first size that will be tested

    private final float INITIAL_SIZE_GROWTH_TAX = (float) 1.0; // initial tax to increase the interval
    private final float RESTART_SIZE_GROWTH_TAX = INITIAL_SIZE_GROWTH_TAX * (float) 0.2; // increase fragment tax after deterioration detected
    private final float REDUCTION_IN_SIZE_GROWTH_TAX = (float) 0.75; // reduction fragment tax when deterioration is detected 
	...
	private int a_greatestNumKeys;

    // Public  Methods ----------------------------------------------

    public PartitionTunerMT_NonUniform(int totalElements, RangeStatistics rangeStatistics) {
        state = INITIAL;
        this.rangeStatistics = rangeStatistics;
        maxNumberOfKeys = calculateNumberOfKeys(this.rangeStatistics.getMaxSizeRatio());
        
        //maxNumberOfKeys = calculateNumberOfKeys(totalElements);
    ...
    public void setSizeResults(PartitionSize size) throws IllegalThreadStateException {
        ...
        case SEARCHING: {
            if (currentNumKeys == 0) {
            ...
            } else if (size.getMeanExecutionTime() <= (meanTimeCurrentNumKeys * (1.0 + MEANTIME_GROWTH_TOLERANCE * sizeGrowthTax))) {
                ...
                sizeGrowthTax = sizeGrowthTax * ((float) 1.0 - REDUCTION_IN_SIZE_GROWTH_TAX);
        ...
        case SET: {
            if (size.numberOfKeys() != currentNumKeys)
                throw new IllegalArgumentException("SET: Size was expected to be equal to current. current = " + currentNumKeys + ". Size = " + size.numberOfKeys());
            numSets++;
        ...
        case TRAVERSING_LOW_DENSITY_REGION: {
            if (size.numberOfKeys() != currentNumKeys)
                throw new IllegalArgumentException(
                        "TRAVERSING_LOW_DENSITY_REGION: Size was expected to be equal to current. current = " + currentNumKeys + ". Size = " + size.numberOfKeys());
            numIterInsideLowDensityRegion++;
            if (size.getMeanExecutionTime() > meanTimeCurrentNumKeys * (1.0 + MEANTIME_LOW_DENSITY_TOLERANCE)) {
        ...
        default:
            throw new IllegalThreadStateException("PartitionTuner at an unknown state: " + state);
        }
    }
    ...
    public PartitionSize getPartitionSize() throws IllegalThreadStateException {
        ...
        case INITIAL: {
           ...
           // a_greatestNumKeys = 0;
            size = new PartitionSize(calculateRatio(INITIAL_NUMBER_OF_ELEMENTS), INITIAL_NUMBER_OF_ELEMENTS, NUMBER_OF_EXECUTIONS);
            state = SEARCHING;
            break;
        }
        case SET: {
            size = new PartitionSize(calculateRatio(currentNumKeys), currentNumKeys, 1);
            break;
        }
        case SEARCHING: {
            int numkeys;
            numkeys = (int) Math.ceil((double) currentNumKeys * (1.0 + sizeGrowthTax));
        ...
        case RESTARTING: {
            int numkeys;
            numkeys = (int) Math.ceil((double) currentNumKeys / (1.0 + SIZE_REDUCTION_WHILE_RESTARTING));
            if (numkeys < 1)
                numkeys = 1;
            size = new PartitionSize(calculateRatio(numkeys), numkeys, NUMBER_OF_EXECUTIONS);
            break;
    ...
    public void printTuningStatistics(PrintStream out) {
        ...
        out.println("The limit would not be respected " + numTimesMaxWouldNotBeRespected + " times.");
        out.println("Number of times a low-density region has been detected: " + numLowDensityDetections);
        out.println("Number of iteractions inside a low-density region: " + numIterInsideLowDensityRegion);
    }
    ...
    private float calculateRatio(int numberOfKeys) {
    	...
    	//System.out.println(rangeStatistics.getMeanNumTuplesPerValue());
    	//System.out.println(rangeStatistics.getTotalNumberOfTuples());
        return (float) ((((float) numberOfKeys) * rangeStatistics.getMeanNumTuplesPerValue()) / rangeStatistics.getTotalNumberOfTuples());
    }

    private int calculateNumberOfKeys(float ratio) {
        return (int) ((ratio * rangeStatistics.getTotalNumberOfTuples()) / rangeStatistics.getMeanNumTuplesPerValue());
    }
