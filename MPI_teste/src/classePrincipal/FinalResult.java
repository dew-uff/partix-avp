...
public class FinalResult {
	...
	public static String getFinalResult(int totalInstances, String basepath, OutputStream out)
		...
		if (q.getAggregateFunctions() != null
				&& q.getAggregateFunctions().size() > 0) { // possui funcoes de
															// agregacao na
															// clausula LET.

			finalResultXquery = sbq.getConstructorElement()
					+ "{ \r\n"
					+ " let $c:= collection('tmpResultadosParciais')/partialResult/"
			...
			for (String function : keys) {
				...
				if (elementsAroundFunction.indexOf("/") != -1) { // o elemento
																	// depois do
																	// return
																	// possui
																	// sub-elementos.
					...
					expression = expression.replace(
							"$c)",
							"$c/"
									+ sbq.elementAfterConstructor.replaceAll(
											"[</>]", "") + ")");
		...
		/*
		 * ExecucaoConsulta exec = new ExecucaoConsulta(); String finalResult =
		 * exec.executeQuery(finalResultXquery);
		 */
		...
		for (String host : allHosts) {
			...
			try {
				...
				// System.out.println("Finalresult.java xquery:"+
				// finalResultXquery);
				boolean res = st.execute(finalResultXquery);

				if (res) {
					SednaSerializedResult rs = st.getSerializedResult();
					// System.out.println("Subquery.executeSubQuery: - rs.next():"
					// + rs.next());
					// String resp = rs.next();
					...
					/*
					 * while ( (resp = rs.next()) != null ) { //retorno =
					 * retorno + rs.next(); retorno = retorno + resp;
					 * System.out.
					 * println("SubQuery.executeSubQuery():retorno="+retorno
					 * +"."); }
					 */
		...
		if (q.getAggregateFunctions() != null
			...
			// System.out.println("SubQuery.storeXMLDocumentIntoCollection(): runningSubqueries:"
			// + sbq.isRunningSubqueries());

			if (ExecucaoConsulta.executeQueryAsString(
					"for $col in doc('$collections')/collections/collection/@name=\'tmpResultadosParciais' return $col",
					"32").equals("true")) {
			...
			st.execute("LOAD '" + path.replace("\\", "/")
					+ "' 'xqueryAnswer.xml' 'tmpResultadosParciais'");
			...
			// not needed since the file already has the whole result.
